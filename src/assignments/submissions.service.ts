...
export class SubmissionsService {
  ...
  async create(createSubmissionDto: CreateSubmissionDto): Promise<Submission> {
    ...
    // IMPORTANT: we deliberately avoid persisting raw storage-provider URLs here.
    // Frontend should send internal file references (e.g. uploads/... paths or file IDs),
    // and view URLs are always generated by the files service.
    const cleanedAttachments =
      createSubmissionDto.attachments && createSubmissionDto.attachments.length > 0
        ? createSubmissionDto.attachments
            .map((att) => (att || '').trim())
            .filter(
              (att) =>
                att &&
                // Ignore obvious presigned URLs that include X-Amz-Algorithm etc.
                !att.includes('X-Amz-Algorithm=') &&
                !att.startsWith('http://') &&
                !att.startsWith('https://'),
            )
        : undefined;

    const cleanedFileUrl =
      createSubmissionDto.fileUrl &&
      !createSubmissionDto.fileUrl.includes('X-Amz-Algorithm=') &&
      !createSubmissionDto.fileUrl.startsWith('http://') &&
      !createSubmissionDto.fileUrl.startsWith('https://')
        ? createSubmissionDto.fileUrl
        : undefined;
    ...
    const submissionData: Partial<Submission> = {
      ...
      // Persist only internal reference (e.g. uploads/... or file ID), never full S3/Blaze URL
