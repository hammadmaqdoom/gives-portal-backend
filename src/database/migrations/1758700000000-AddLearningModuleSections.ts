...
export class AddLearningModuleSections1758700000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasSectionId = (await queryRunner.getTable('learning_module'))?.columns.find(c => c.name === 'sectionId');
    if (!hasSectionId) {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const fk = lmTable?.foreignKeys.find((f) => f.columnNames.includes('sectionId'));
    if (fk) {
